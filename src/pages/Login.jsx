...
export default function Login() {
  ...
  const inputStyle = {
    ...
    border: '1.5px solid rgba(255,255,255,0.05)', outline: 'none', fontSize: '0.9rem',
    color: '#f8fafc', backgroundColor: 'rgba(255,255,255,0.03)', transition: 'all 0.2s ease',
    fontFamily: 'inherit'
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: '460px' }}>
          ...
          <p style={{ fontSize: '1.125rem', color: '#94a3b8', lineHeight: 1.6, marginBottom: '2rem', fontWeight: '400' }}>
          ...
          <div style={{ display: 'flex', gap: '3rem' }}>
            {[
              ...
              <div key={i}>
                <div style={{ color: '#475569', marginBottom: '0.5rem' }}>{m.icon}</div>
                <div style={{ fontSize: '1.75rem', fontWeight: '800', color: '#f8fafc', letterSpacing: '-0.03em' }}>{m.value}</div>
                <div style={{ fontSize: '0.875rem', color: '#64748b', fontWeight: '500', marginTop: '0.25rem' }}>{m.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '0.875rem', color: '#475569' }}>© 2026 PT. Kawan Lama</span>
          <span style={{ fontSize: '0.875rem', color: '#475569' }}>Powered by YOLO11</span>
        </div>
      </div>
      ...
      <div style={{
        ...
        {/* Soft blur element in background for form panel */}
        <div style={{ position: 'absolute', top: '20%', left: '20%', width: 300, height: 300, background: 'rgba(59, 130, 246, 0.05)', borderRadius: '50%', filter: 'blur(80px)', zIndex: 0 }} />

        <div style={{
          width: '100%', maxWidth: '420px', backgroundColor: 'rgba(255, 255, 255, 0.02)',
          ...
          {(mode === 'login' || mode === 'signup') && (
            <div style={{ display: 'flex', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: '20px', padding: '4px', marginBottom: '2rem', border: '1px solid rgba(255,255,255,0.02)' }}>
              {['login', 'signup'].map(m => (
          ...
          <div style={{ marginBottom: '2rem' }}>
            <h2 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#f8fafc', marginBottom: '0.5rem', letterSpacing: '-0.025em' }}>
          ...
          {/* Alerts */}
          {error && (
            <div style={{ backgroundColor: 'rgba(239, 68, 68, 0.1)', color: '#fca5a5', padding: '0.875rem 1rem', borderRadius: '12px', fontSize: '0.875rem', marginBottom: '1.5rem', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
              {error}
            </div>
          )}
          {successMsg && (
            <div style={{ backgroundColor: 'rgba(34, 197, 94, 0.1)', color: '#86efac', padding: '0.875rem 1rem', borderRadius: '12px', fontSize: '0.875rem', marginBottom: '1.5rem', border: '1px solid rgba(34, 197, 94, 0.2)' }}>
              {successMsg}
            </div>
          )}
          {mode === 'reset' && demoCode && (
            <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', padding: '0.875rem 1rem', borderRadius: '12px', fontSize: '0.875rem', marginBottom: '1.5rem', border: '1px solid rgba(255,255,255,0.05)', color: '#f8fafc' }}>
              <span style={{ color: '#94a3b8' }}>Your code: </span>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.8125rem', fontWeight: '600', marginBottom: '0.5rem', color: '#e2e8f0' }}>Email</label>
              <input type="email" placeholder="manager@kawanlama.com" value={email}
                ...
                style={{ ...inputStyle, opacity: mode === 'reset' ? 0.5 : 1 }}
            ...
            {(mode === 'login' || mode === 'signup') && (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  <label style={{ fontSize: '0.8125rem', fontWeight: '600', color: '#e2e8f0' }}>Password</label>
                  {mode === 'login' && (
                    <button type="button" onClick={() => { setMode('forgot'); setError(''); setSuccessMsg(''); }}
                      style={{ fontSize: '0.8125rem', color: '#60a5fa', background: 'none', border: 'none', fontWeight: '500', cursor: 'pointer', fontFamily: 'inherit' }}
            ...
            {mode === 'reset' && (
              <>
                <div>
                  <label style={{ display: 'block', fontSize: '0.8125rem', fontWeight: '600', marginBottom: '0.5rem', color: '#e2e8f0' }}>Verification code</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.8125rem', fontWeight: '600', marginBottom: '0.5rem', color: '#e2e8f0' }}>New password</label>
            ...
            {mode === 'login' && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.25rem' }}>
                <span style={{ fontSize: '0.875rem', color: '#94a3b8', fontWeight: '500' }}>Remember me</span>
                <div
                  onClick={() => setRememberMe(!rememberMe)}
                  style={{
                    width: 44, height: 24, borderRadius: 12,
                    backgroundColor: rememberMe ? '#3b82f6' : 'rgba(255,255,255,0.1)',
                ...
                >
                  <div style={{
                    ...
                    position: 'absolute', top: 1, left: rememberMe ? 21 : 1,
                    transition: 'left 0.2s cubic-bezier(0.4, 0.0, 0.2, 1)', boxShadow: '0 2px 4px rgba(0,0,0,0.2)'
                  }} />
                </div>
              </div>
            )}
            ...
            <button type="submit" disabled={isLoading}
              ...
              onMouseOver={(e) => { if (!isLoading) { e.currentTarget.style.backgroundColor = '#1d4ed8'; e.currentTarget.style.transform = 'translateY(-1px)'; }}}
            ...
            {(mode === 'forgot' || mode === 'reset') && (
              <button type="button" onClick={() => { setMode('login'); setError(''); setSuccessMsg(''); setDemoCode(''); }}
                style={{ width: '100%', padding: '0.5rem', backgroundColor: 'transparent', color: '#94a3b8', border: 'none', fontSize: '0.875rem', fontWeight: '500', cursor: 'pointer', fontFamily: 'inherit' }}
          ...
          {(mode === 'login' || mode === 'signup') && (
            <div style={{ marginTop: '2rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', margin: '1.5rem 0' }}>
                ...
                <span style={{ padding: '0 0.75rem', fontSize: '0.75rem', color: '#64748b', fontWeight: '500' }}>or connect with</span>
                <div style={{ flex: 1, height: '1px', backgroundColor: 'rgba(255,255,255,0.05)' }} />
              </div>
              
              <div style={{ display: 'flex', gap: '1rem' }}>
                <button type="button" onClick={() => alert('SSO Demo Mode')} style={{
                  flex: 1, padding: '0.625rem', backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)',
                  borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
                  fontSize: '0.875rem', fontWeight: '500', color: '#e2e8f0', cursor: 'pointer', transition: 'all 0.2s ease'
                }}
